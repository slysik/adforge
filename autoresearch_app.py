...
Usage:
    python3 autoresearch_app.py              # Continuous loop
    python3 autoresearch_app.py --once       # Single cycle
    python3 autoresearch_app.py --cycles 5   # Run N cycles
    python3 autoresearch_app.py --reset      # Reset state + single cycle
"""
...
BASE_DIR = Path(__file__).resolve().parent / "data" / "autoresearch_app"
...
SLIDE = {
    ...
    "description": (
        ...
        "Subtitle below in smaller marker: 'From 2,208 lines to 4 focused modules'. "
        "\n\n"
        ...
        "Subheader: '236 lines — Orchestrator' in smaller text. "
        "Below, FOUR bullet items with small icons: "
        "  gear icon — 'Page config + CSS injection', "
        "  database icon — 'Session state init (6 keys)', "
        ...
        "Subheader: '449 lines — Widgets + Constants' in smaller text. "
        "Below, FOUR bullet items with small icons: "
        ...
        "Subheader: '1,196 lines — Page Renderers' in smaller text. "
        "Below, FOUR bullet items with small icons: "
        "  form icon — 'render_brief_builder() — 4-tab wizard', "
        ...
        "  'st.session_state persists across reruns (25 keys)', "
...
5. CODE_KNOWLEDGE (0-4): The slide demonstrates that the presenter deeply understands the codebase — not just high-level boxes. Specific evidence of depth: exact line counts, key function names (render_metric_cards, render_brief_builder, score_asset), awareness of Streamlit constraints (set_page_config must be first, session_state keys, rerun semantics), the import dependency direction, and why CSS is a raw file (IDE support). An interviewer would believe this person wrote and refactored this code.
...
CONTEXT: AdForge — a Streamlit web app for creative automation. The monolithic app.py (2,208 lines) was refactored into 4 focused modules. This slide shows the modular architecture for a technical interview — proving deep code knowledge.

COLORS:
- app.py box: light coral (#ffc9c9) — it's the entry point / orchestrator.
...
- app.py (236 lines): Page config, CSS injection, session state init (6 keys), sidebar nav, pipeline execution gate
- app_components.py (449 lines): render_metric_cards(), render_pipeline_stepper(), score_asset(), TEMPLATE_INFO, PIPELINE_STAGES
- app_pages.py (1,196 lines): render_brief_builder() 4-tab wizard, render_approval_queue(), render_performance() KPI table, render_pipeline_results()
- app_styles.css (742 lines): CSS custom properties (brand tokens), .af-card/.af-hero component classes, Streamlit overrides
- Bottom banner: Streamlit runtime — session_state persists across reruns, module cache, st.set_page_config() must be first call

IMPORT ARROWS: app.py → app_pages.py → app_components.py (left-to-right). app.py reads app_styles.css (dashed arrow down). No circular dependencies.

LAYOUT LAW: STRICT left-to-right primary flow for the 4 module boxes. Bottom banner spans full width below. ALL arrows point RIGHT or DOWN only.
