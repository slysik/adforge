...
Usage:
    ...
    python3 autoresearch.py --reset      # Reset state + single cycle
"""
...
BASE_DIR = Path(__file__).resolve().parent / "data" / "autoresearch"
...
SLIDE = {
    ...
    "description": (
        ...
        "\n\n"
        "INPUT (left edge): A box labeled 'Campaign Brief (YAML)' with a document icon. Pastel orange fill. "
        "\n\n"
        ...
        "BOX 2 (light green fill): 'Analyze Brief' — brain/lightbulb icon. Below: 'analyzer.py — Heuristic + LLM'. "
        "BOX 3 (light yellow fill): 'Resolve Cache' — database/magnifier icon. Below: 'storage.py — Hero Reuse'. "
        "BOX 4 (light purple fill): 'Generate Heroes' — magic wand/sparkle icon. Below: 'providers.py — Firefly / Gemini / Mock'. "
        "BOX 5 (light pink fill): 'Compose Creatives' — paint palette/layers icon. Below: 'compositor.py + templates.py — Pillow'. "
        "BOX 6 (light teal fill): 'Validate Compliance' — shield/checkmark icon. Below: 'validator.py — Pixel + Text'. "
        "BOX 7 (light gray fill): 'Report' — chart/dashboard icon. Below: 'report.py — HTML / JSON'. "
        "\n\n"
        "OUTPUT (right edge): A box labeled '18 Localized Creatives' with a grid/gallery icon. Pastel orange fill. "
        "\n\n"
...
def generate_slide(gemini_client, prompt: str, output_path: Path) -> bool:
    ...
    full_prompt = (
        ...
        f"  - 'Pydantic' 'Pillow' 'Firefly' 'Gemini' 'ThreadPoolExecutor'\n"
        f"  - 'Heuristic' 'Compliance' 'YAML'\n\n"
        f"--- SLIDE TO CREATE ---\n"
        f"{SLIDE['title']}\n"
...
CONTEXT: AdForge — a creative automation pipeline that transforms a single campaign brief into 18 localized ad creatives in seconds. This is the solution architecture overview for a technical interview.
